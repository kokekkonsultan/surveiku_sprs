...
class AnalisaSurveiController extends Client_Controller
{
	...
	public function detail_pelayanan($id1, $id2, $id3)
	{
		...
		$this->data['layanan'] = $this->db->get()->row();
		...
		WHERE survey_$table_identity.is_submit = 1 AND responden_$table_identity.id_layanan_survei = $id3
		GROUP BY id_sub");
		...
		return view('analisa_survei/detail_pelayanan', $this->data);
	}
	...
	public function ajax_list()
	{
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Models->count_all($table_identity, $id_layanan_survei),
			"recordsFiltered" => $this->Models->count_filtered($table_identity, $id_layanan_survei),
			"data" => $data,
		);
	...
	public function edit($id1, $id2)
	{
		...
		$this->data['id_layanan_survei'] = $current->id_layanan_survei;
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->db->affected_rows() > 0) {
				...
				// redirect(base_url() . $this->session->userdata('username') . '/' . $this->uri->segment(2) . '/analisa-survei', 'refresh');
				redirect(base_url() . $this->session->userdata('username') . '/' . $this->uri->segment(2) . '/analisa-survei/detail-pelayanan/'.$input['id_layanan_survei'], 'refresh');
			}
		}
	}
	...
	public function detail_unsur($username, $slug_manage_survey, $id_unsur_pelayanan)
	{
		...
		$cek_current = $this->db->get_where("analisa_$table_identity", ["id_unsur_pelayanan" => $this->uri->segment(5), "id_layanan_survei" => $this->uri->segment(6)]);
        ...
        $this->db->select("*, jawaban_pertanyaan_unsur_$table_identity.id AS id_jawaban_pertanyaan_unsur, survey_$table_identity.id AS id_saran, (SELECT nama_kategori_unsur_pelayanan FROM kategori_unsur_pelayanan_$table_identity WHERE nomor_kategori_unsur_pelayanan = jawaban_pertanyaan_unsur_$table_identity.skor_jawaban && id_pertanyaan_unsur = jawaban_pertanyaan_unsur_$table_identity.id_pertanyaan_unsur) AS bobot, $query_profil");
        $this->db->from("jawaban_pertanyaan_unsur_$table_identity");
	...
	public function ajax_list_analisa($username, $slug_manage_survey, $id_unsur_pelayanan, $id_layanan_survei)
	{
		...
		// $id_layanan_survei = $this->uri->segment(6);

		$profil_responden = $this->db->query("SELECT * FROM profil_responden_$table_identity")->result();

		$list = $this->Analisa_model->get_datatables($table_identity, $id_unsur_pelayanan, $profil_responden, $id_layanan_survei);
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $value) {
			...
			if($value->alasan_pilih_jawaban) {
				...
				$button_edit = '<button type="button" class="btn btn-light-primary btn-sm" data-toggle="modal" data-target="#edit' . $value->id_jawaban_pertanyaan_unsur . '"><i class="fa fa-edit"></i></button>';
			} else {
				$button_edit = '';
			}

			if($value->saran) {
				$button_edit_saran = '<button type="button" class="btn btn-light-primary btn-sm" data-toggle="modal" data-target="#edit_saran' . $value->id_saran . '"><i class="fa fa-edit"></i></button>';
			} else {
				$button_edit_saran = '';
			}
			...
			$row[] = $value->alasan_pilih_jawaban.'<br>'.$button_edit;
			// $row[] = '<button type="button" class="btn btn-light-primary btn-sm" data-toggle="modal" data-target="#edit' . $value->id_jawaban_pertanyaan_unsur . '"><i class="fa fa-edit"></i> Edit</button>';
			$row[] = $value->saran.'<br>'.$button_edit_saran;
			// $row[] = $status;
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Analisa_model->count_all($table_identity, $id_unsur_pelayanan, $profil_responden, $id_layanan_survei),
			"recordsFiltered" => $this->Analisa_model->count_filtered($table_identity, $id_unsur_pelayanan, $profil_responden, $id_layanan_survei),
			"data" => $data,
		);
	...
	public function edit_alasan()
	{
		...
		$this->session->set_flashdata('message_success', 'Berhasil mengubah data');
		redirect($this->session->userdata('username') . '/' . $this->uri->segment(2) . '/analisa-survei/detail-unsur/' . $this->uri->segment(5) . '/' . $this->uri->segment(6), 'refresh');
	}

	public function edit_saran()
	{
		$table_identity = $this->get_table_identity($this->uri->segment(2));
		...
		// var_dump($object);
		$this->db->where('id',  $this->input->post('id_saran'));
	...
	public function _get_data_profile($id1, $id2)
	{
		...
		$this->db->select('u1489187_auth.users.username, manage_survey.survey_name, is_question, manage_survey.slug, manage_survey.description, manage_survey.is_privacy, manage_survey.table_identity, manage_survey.id_jenis_pelayanan, manage_survey.atribut_pertanyaan_survey, executive_summary, skala_likert');
