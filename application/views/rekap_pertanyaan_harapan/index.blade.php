                ...
                <div class="card-body d-flex align-items-center">
                    <div>
                        <h3 class="text-white font-weight-bolder line-height-lg mb-5">
                            REKAPITULASI JAWABAN PERTANYAAN HARAPAN
                        </h3>
                        ...
                        btn-primary btn-sm font-weight-bold dropdown-toggle" data-toggle="dropdown"
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                <a class="dropdown-item text-secondary"
                                    href="<?= base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/rekap-pertanyaan-harapan/cetak' ?>"
                                    target="_blank"><i class=" fas fa-file-pdf text-dark"></i>&nbsp;&nbsp;
                                    <b>PDF</b></a>

                                <a class="dropdown-item text-secondary"
                                    href="{{ base_url() }}{{ $ci->session->userdata('username') }}/{{ $ci->uri->segment(2) }}/rekap-pertanyaan-harapan/download-docx"
...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/rekap-pertanyaan-harapan/ajax-list' ?>",
            "type": "POST",
            "data": function(data) {}
        },
