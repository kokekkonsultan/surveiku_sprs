            ...
            <td style="font-weight:bold;" width="95%">
                ...
                <div class="{{$model_ta}}-inline mb-2">
                    <label class="{{$model_ta}} {{$model_ta}}-outline {{$model_ta}}-success {{$model_ta}}-lg" style="font-size: 16px;">
                        <input type="{{$model_ta}}" name="jawaban_pertanyaan_terbuka[{{ $row_terbuka_atas->id_pertanyaan_terbuka }}][]" value="{{ $value_terbuka->pertanyaan_ganda }}" class="terbuka_{{ $row_terbuka_atas->id_pertanyaan_terbuka }}" {{ $is_required_ta_i }} <?= in_array($value_terbuka->pertanyaan_ganda, unserialize($row_terbuka_atas->jawaban)) ? 'checked' : ''; ?>>
