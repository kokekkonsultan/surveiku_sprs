        ...
        <div class="col-md-12">
            ...
            <div class="card" data-aos="fade-down">
                ...
                <div class="card-body">
                    ...
                    <form action="{{base_url() . $ci->uri->segment(1). '/kelompok-akun/edit/' . $ci->uri->segment(4)}}" method="POST">

                    @php
                    $user_id = $ci->session->userdata('user_id');
                    $kelompok_akun = $ci->db->get_where("kelompok_anak_induk_$table_identity", ['id' => $ci->uri->segment(4)])->row();
                    $id_objek = unserialize($kelompok_akun->id_objek);
                    @endphp
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="nama_kelompok" value="{{$kelompok_akun->nama_kelompok}}" class="form-control" required autofocus>
                        </div>
                    </div>
                    <br>
                            ...
                            <select id="id_survei" name="id_survei[]" class="form-control" multiple="multiple" required>
                                ...
                                <optgroup label="{{$row->first_name . ' ' . $row->last_name}}">
                                    @foreach($ci->db->get_where('manage_survey', ['id_user' => $row->id])->result() as $value)
                                        <option value="{{$value->id}}" <?= in_array($value->id, $id_objek) ? 'selected' : '' ?>>{{$value->survey_name}}</option>
                                    @endforeach
                                </optgroup>
                                @endforeach
                                </select>
