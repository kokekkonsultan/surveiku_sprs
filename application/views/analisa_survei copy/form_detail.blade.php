...
<div class="container-fluid mt-5">
    ...
    <a href="{{ base_url() }}{{ $ci->session->userdata('username'); }}/{{ $ci->uri->segment(2) }}/analisa-survei/detail-pelayanan/{{ $ci->uri->segment(6) }}"
    ...
    <div class="card card-custom card-sticky mt-5">
        ...
        <div class="card-body">
            ...
            <input type="hidden" name="id_layanan_survei" value="{{ $ci->uri->segment(6) }}">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/analisa-survei/edit-alasan'. '/' . $ci->uri->segment(5). '/' . $ci->uri->segment(6) ?>"
                    method="POST">
...
<div class="modal fade" id="edit_saran{{$row->id_saran}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-secondary">
            ...
            <div class="modal-body">
                <form action="<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/analisa-survei/edit-saran'. '/' . $ci->uri->segment(5). '/' . $ci->uri->segment(6) ?>"
                    method="POST">

                    <input name="id_saran" value="{{$row->id_saran}}" hidden>

                    <div class="form-group">
                        <label class="form-label font-weight-bold">Saran<span style="color:red;">*</span></label>
                        <textarea id="saran" name="saran" class="form-control" required>{{ $row->saran }}</textarea>
                    </div>
...
$(document).ready(function() {
    table = $('#table_unsur_<?php echo $row->id ?>').DataTable({
        ...
        "ajax": {
            "url": "<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/analisa-survei/ajax-list-analisa/' . $row->id . '/' . $ci->uri->segment(6) ?>",
            "type": "POST",
            "data": function(data) {
...
$(document).ready(function() {
    table = $('#table_unsur').DataTable({
        ...
        "ajax": {
            "url": "<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/analisa-survei/ajax-list-analisa/' . $ci->uri->segment(5). '/' . $ci->uri->segment(6) ?>",
            "type": "POST",
            "data": function(data) {
