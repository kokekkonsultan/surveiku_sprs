            ...
            <div class="card card-custom card-sticky mb-5">
                <div class="card-body">
                    ...
                    <table class="table table-bordered table-striped example" style="width:100%">
                        ...
                        <tbody>
                            @php
                            $wilayah = $ci->db->query("SELECT *
                            FROM (SELECT *,
                            (SELECT COUNT(*) FROM survey_$table_identity WHERE is_submit = 1) AS total_survei,
                            (SELECT COUNT(*) FROM responden_$table_identity JOIN survey_$table_identity ON responden_$table_identity.id = survey_$table_identity.id_responden WHERE wilayah_survei_$table_identity.id = responden_$table_identity.id_wilayah && is_submit = 1) AS perolehan
                            ...
                            $no = 1;
                            @endphp
                            @foreach ($wilayah->result() as $value)
...
<script>
    FusionCharts.ready(function() {
        var myChart = new FusionCharts({
            ...
            dataSource: {
                "chart": {
                    caption: "Wilayah Survei",
                    subcaption: "Cakupan Wilayah yang di Survei",
                    "enableSmartLabels": "1",
                    "startingAngle": "0",
                    "showPercentValues": "1",
                    "decimals": "2",
